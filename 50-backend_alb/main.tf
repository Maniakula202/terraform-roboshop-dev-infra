resource "aws_lb" "backend_alb" {
    name               = "${var.project_name}-${var.environment}-backend-alb" # roboshp-dev-backend_alb
    internal           = true
...
resource "aws_route53_record" "backend_alb" {
  zone_id = var.zone_id
  name    = "*.backend-alb-${var.environment}.${var.domain_name}"  # *.backend-alb-dev.manidevops.fun
  type    = "A"
